...
def ma_strategy(data):
    ...
    df['Sell'] = np.where(
                          ...
                          (df.Close.astype(float) > df['buy_price'].astype(float)),
                          df.Close, "NaN")
    ...
    return df['Buy'], df['Sell_single'], df['tot_buy'], df['tot_sell']
...
def create_dash(flask_app):
        ...
        app.layout = html.Div([
            ...
            ##### Store data
             ...
             #### Header 
                ...
                html.Div([
                  dcc.Textarea(id = 'score_lose',
                               style = {'color': 'white  ',
                                        'height': '20px'},
                               value='you lose:',
                               className = 'stock_score_lose'),
                  ]),  
        ...
        def update_df(n_intervals): 
                if n_intervals == 0:
                    ...
                    #filename = "2022-07-08 NQ=F_out_stock_data.csv"
                    cwd = os.getcwd()
        ...
        def update_graph(n_intervals, data):
            ...
            if n_intervals == 0:
                ...
                buy, sell, tot_buy, tot_sell = ma_strategy(df)
                #profit = df.tot_buy.tail(1).iloc[0].apply(Decimal) - df.tot_sell.apply(Decimal)
