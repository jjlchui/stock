...
def ma_strategy(data):
    ...
    return df['Buy'], df['Sell_single'], df['tot_buy'], df['tot_sell'], df['Buy_count'], df['Sell_count']
...
def create_dash(flask_app):
        ...
        def update_graph(n_intervals, data):
            ...
            if n_intervals == 0:
                ...
                #df.profit = df.tot_buy.tail(1).iloc[0].apply(Decimal) - df.tot_sell.tail(1).iloc[0].apply(Decimal)
                tt_buy = tot_buy.iloc[-1]
                #tt_buy = tt_buy.transform(lambda x: f"check {' ' * x} : left blank")
