...
def create_dash(flask_app):
        ...
        app=dash.Dash(server=flask_app, name="stock_dash", url_base_pathname=("/mamacd/"), prevent_initial_callbacks=True)
        ...
        def update_df(n_intervals): 
                if n_intervals == 0:
                    ...
                    #filename = "2022-08-19 NQ=F USTime_out_stock_data.csv"
                    cwd = os.getcwd()
        ...
        def update_graph(n_intervals, data):
            ...
            if n_intervals == 0:
                ...
                buy_list = []
                buy_idx = []
                for idx, xs in enumerate(buy_macd):
                    if xs != 'NaN':
                        buy_list.append(xs)
                        buy_idx.append(idx)
                ...
                sell_list = []
                sell_idx = []
                for idx, xs in enumerate(sell_macd):
                    if xs != 'NaN':
                        sell_list.append(xs)
                        sell_idx.append(idx)
